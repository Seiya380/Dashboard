...
import { watchlistTickers, settings } from '$lib/server/db/schema.js';
...
async function refreshYahooSession(): Promise<boolean> {
	try {
		// 1. Load saved session from DB
		const [row] = await db.select().from(settings).where(eq(settings.key, 'yf_session'));
		if (row) {
			const saved = JSON.parse(row.value) as { cookie: string; crumb: string; ts: number };
	...
	// 2. Fetch fresh cookies from Yahoo Finance homepage
	try {
		...
		await db
			.insert(settings)
			.values({ key: 'yf_session', value: JSON.stringify({ cookie: cookies, crumb, ts: _sessionTs }), updatedAt: ts })
			.onConflictDoUpdate({ target: settings.key, set: { value: JSON.stringify({ cookie: cookies, crumb, ts: _sessionTs }), updatedAt: ts } });
...
const quoteCache = new Map<string, { quote: TickerQuote; ts: number }>();
const CACHE_TTL = 60_000;

// ── CoinGecko for crypto ───────────────────────────────────────────────────

// Maps common Yahoo Finance crypto symbols → CoinGecko IDs
const COINGECKO_IDS: Record<string, string> = {
	'BTC-USD': 'bitcoin', 'BTC-EUR': 'bitcoin',
	'ETH-USD': 'ethereum', 'ETH-EUR': 'ethereum',
	'SOL-USD': 'solana', 'SOL-EUR': 'solana',
	'BNB-USD': 'binancecoin', 'BNB-EUR': 'binancecoin',
	'XRP-USD': 'ripple', 'XRP-EUR': 'ripple',
	'ADA-USD': 'cardano', 'ADA-EUR': 'cardano',
	'DOGE-USD': 'dogecoin', 'DOGE-EUR': 'dogecoin',
	'AVAX-USD': 'avalanche-2', 'AVAX-EUR': 'avalanche-2',
	'MATIC-USD': 'matic-network', 'MATIC-EUR': 'matic-network',
	'DOT-USD': 'polkadot', 'DOT-EUR': 'polkadot'
};
...
async function fetchCryptoQuote(symbol: string): Promise<TickerQuote> {
...
async function fetchYahooQuote(symbol: string, retry = true): Promise<TickerQuote> {
	...
	try {
		...
		// 429 → refresh session and retry once
		if (res.status === 429 && retry) {
			_sessionTs = 0; // invalidate
...
async function fetchQuote(symbol: string): Promise<TickerQuote> {
	return isCrypto(symbol) ? fetchCryptoQuote(symbol) : fetchYahooQuote(symbol);
}

async function fetchQuotes(symbols: string[]): Promise<Map<string, TickerQuote>> {
	const result = new Map<string, TickerQuote>();
	// Stagger stock requests 300ms apart; crypto can go in parallel
...
export const POST = async ({ request }) => {
	...
	const [row] = await db
		...
		.values({ id, symbol: sym, displayName: displayName?.trim() || null, createdAt: now() })
		.onConflictDoNothing()
		.returning();
